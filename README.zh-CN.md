...
它复现的是我在公司做过的一个系统：**把本地服务器的 syslog 采集上云、存储并告警**。
那份数据不能出内网，所以这里把数据源换成公开天气 API，其余部分——采集节奏、缓冲层、
原始层与加工层的分离、阈值告警、运维看板——完整保留。

这个替换是经过考虑的，不是随手找了个 API。天气读数和日志数据共享几个让人头疼的性质：

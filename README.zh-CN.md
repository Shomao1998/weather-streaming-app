...
我参与过一个上云后存储网络 syslog 的需求。最初提议以 Azure 存储承载日志，并用
Application Insights 监控传输异常，以满足日志零丢失与至少两年留存的合规要求；
该方案因 TB 级体量下的存储与监控费用未能推进。我另负责跨团队任务进度追踪及看板呈现。
参考公开作品集，本项目将两者合并为一套天气流式采集与监控看板。

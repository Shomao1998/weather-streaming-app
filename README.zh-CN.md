...
它把我在一家金融企业做过的两件事拼在了一起——当时那家公司正在从自建数据中心上云。

**第一件是一个提案**：把本地服务器的 syslog 采集到 Azure、存储并告警。
**这个方案最终没有被采纳。** 合规要求日志至少保留两年，而以那些服务器的日志体量，
存储加监控的账单让整个商业论证不成立。**架构本身没问题，是经济账算不过来。**

**第二件是当时手上的日常工作**：内部工作流自动化，以及为它做报告的看板维护。

这个项目就是这两件事的合体，用一个免费的公开天气 API 顶替日志源。
真实数据不能出内网——而且当初压垮那个提案的数据体量，本来也不该由一个作品集项目去复现。

**它是怎么做出来的。** 这是一个 vibe coding 项目：需求、约束和「做完」的标准由我定，
关键取舍由我拍板——留什么、砍什么、什么值得花这笔钱；实现部分则是与 AI 编码代理反复迭代写出来的。
下面每一个决策都写明了理由而不是只给结论，**这样它们是可以被质疑的，而不是只能被相信。**

**这段经历正是这份 README 里会出现成本章节的原因。** 每个组件旁边都标着月度费用，
唯一昂贵的组件被做成了一个配置开关就能关掉，每一层的保留期都是一个写明的决定而不是默认值。
**"能跑，但没人付得起运行费用"是数据平台真实的死法之一**，而这个想法的第一个版本就是这么死的。
...
**只增不减的存储，正是一个日志平台死于成本而非架构的方式。** 每一层的保留策略都写在
`infra/main.bicep` 里，而不是留在某个人的脑子里：
